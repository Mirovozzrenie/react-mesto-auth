...
return(
    <PopupWithForm name={`profile-editor`} title={'Редактировать профиль'} isOpen={props.isOpen}
        ...
        <input className="popup__user-name popup__input-field" onChange={handleChangeName} value={name || ""} type="text" placeholder="Фамилия" required
               name="name" autoComplete="off" minLength="2" maxLength="40" id="user-name"/>
        <span id="user-name-error" className="popup__error"></span>
        <input className="popup__user-status popup__input-field" onChange={handleChangeDescription} value={description || ""} type="text" placeholder="Статус" required
